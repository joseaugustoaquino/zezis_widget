...
import 'package:zezis_widget/src/components/rx_dart/src/transformers/backpressure/backpressure.dart';
...
/// buffer as a [Stream] and clears it, and starts a buffer each
...
///
/// ### Example
/// if startBufferEvery is 2, then a buffer will be started
/// on every other value from the source. A buffer is started at the
...
class WindowCountStreamTransformer<T>
  ...
  ///
  /// A buffer is created for every n-th event emitted
  /// by the [Stream] that is being transformed, as specified by
  /// the [startBufferEvery] parameter.
  ///
  /// If [startBufferEvery] is omitted or equals 0, then a buffer is started whenever
...
extension WindowExtensions<T> on Stream<T> {
  ...
  /// the buffer as a [Stream] and clears it, and starts a buffer each
  /// [startBufferEvery] values. If [startBufferEvery] is not provided, then new
  /// buffers are started immediately at the start of the source and when each
  /// buffer closes and is emitted.
  ///
